...
st.set_page_config(page_title='Ask the Latin Music Theorists', page_icon='🔎')

st.sidebar.header('About this App 🔎')
st.title('🔎 Ask the Latin Music Theorists')
st.write("This Streamlit application allows you to query a database of Latin music theory texts using a large language model (LLM) with retrieval-augmented generation (RAG). Learn more about the system and how to write effective prompts with the tools at the left.") 
...
if sources:
    ...
    data =pd.read_csv("latin_tei_metadata.csv").to_dict(orient="records")
...
# @st.cache_resource
@st.cache_resource
def load_chroma_vector_store(persist_directory: str, collection_name: str, _embeddings):
    """
    Load ChromaDB vector store with proper embeddings function.
    Note: _embeddings parameter starts with underscore to exclude from caching hash
    """
    try:
        vector_store = Chroma(
            collection_name=collection_name,
            embedding_function=_embeddings,  # Pass the actual embeddings object
...
if not openai_api_key.startswith("sk-"):
    ...
    if vector_store:
        st.success("✅ Latin ChromaDB loaded successfully from cache")
    else:
        st.error("❌ Failed to load ChromaDB.")
        st.stop()
    
    # function to get unique authors from db
    ...
    # Get available authors from database
    ...
    # Add "Select All" button - this will work regardless of current selection
    # Add "Select All" button - this will work regardless of current selection
    ...
    if st.sidebar.button("Clear All Authors"):
        st.session_state.selected_authors = []
        st.rerun()

    # Multiselect widget - uses session state for default
    selected_authors = st.sidebar.multiselect(
        "Filter by Author",
        options=available_authors,
        default=st.session_state.selected_authors,
        help="Select which authors to search. All authors are selected by default."
    )

    # Update session state ONLY when the multiselect value actually changes
    ...
    def retrieve(state: State):
        ...
        """
        # Get selected authors from session state
    ...
    if st.session_state.response:
        ...
        for doc in st.session_state.response['context']:
            ...
            st.info(f"Source: {metadata.get('source', 'Unknown')}")
            st.info(f"Page Number: {metadata.get('page_umber', 1)}")
        ...
        st.sidebar.download_button(
            ...
            data=pdf_buffer,
            file_name=f"latin_music_theory_query_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf",
